...
from Lyka import YouTube, app
from Lyka.core.call import Lyka
from Lyka.misc import SUDOERS, db
...
from Lyka.utils.decorators.language import languageCB
from Lyka.utils.formatters import seconds_to_min
from Lyka.utils.inline import (
    close_markup,
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Lyka.pause_stream(chat_id)
        ...
        await CallbackQuery.answer()
        await Lyka.stop_stream(chat_id)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await Lyka.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Lyka.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await Lyka.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            ...
            try:
                await Lyka.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Lyka.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
